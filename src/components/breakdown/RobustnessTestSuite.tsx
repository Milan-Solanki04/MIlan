    ...
    <Card variant="default" padding="none" className="overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          <div className="h-8 w-8 rounded-md bg-emerald-50 text-emerald-700 border border-emerald-200 flex items-center justify-center shrink-0">
        ...
        <div className="flex items-center gap-2 text-slate-400">
          <span className="text-xs font-medium hidden sm:inline text-slate-500">
            {isOpen ? "Hide Suite" : "Explore 12 Tests"}
          </span>
      ...
      {/* Expanded Test Grid */}
      {isOpen && (
        <div className="p-4 sm:p-5 border-t border-border bg-slate-50/50 space-y-4">
          {/* Navigation Tabs */}
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-3">
            <div className="flex items-center gap-1 p-1 bg-slate-200/70 rounded-md text-xs font-medium">
              <button
                type="button"
            ...
            {activeTab === "AUTOMATED_TESTS" && (
              ...
              >
                {isRunningAllTests ? "Running 31 Tests..." : "Run Test Suite (npm test)"}
              </Button>
            )}
          </div>
          ...
          {activeTab === "AUTOMATED_TESTS" && (
            <div className="space-y-4">
              ...
              <Card variant="default" padding="md" className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant={testRunResults ? "success" : "neutral"} size="sm">
                ...
                <div className="flex items-center gap-4 text-xs font-mono">
                  <div className="text-right">
                    <span className="text-[10px] text-slate-400 block uppercase">CLI Equivalent</span>
                    <span className="font-semibold text-slate-800 bg-slate-100 px-2 py-0.5 rounded">npm test</span>
                  </div>
                  {testRunResults && (
                    <div className="text-right">
                      <span className="text-[10px] text-slate-400 block uppercase">Duration</span>
                      <span className="font-semibold text-emerald-700">{testRunResults.duration_ms}ms</span>
                    </div>
                  )}
                </div>
              </Card>
              ...
              {!testRunResults && (
                <div className="p-6 rounded-md border border-dashed border-border bg-white text-center space-y-2">
                  <Cpu className="h-6 w-6 text-slate-400 mx-auto" />
                  <h4 className="text-xs font-semibold text-slate-800">Deterministic Test Suite Standby</h4>
                  ...
                  <Card variant="default" padding="sm" className="space-y-2">
                    ...
                    <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1">
                      {testRunResults.suites.backend_services.map((t, idx) => (
                        <div key={idx} className="text-[11px] p-2 rounded bg-slate-50 border border-border flex items-start justify-between gap-1.5">
                          <div className="flex items-start gap-1.5">
                            <CheckCircle2 className="h-3.5 w-3.5 text-emerald-600 shrink-0 mt-0.5" />
                            <div>
                              <span className="font-semibold text-slate-800 block">{t.suite}</span>
                              <span className="text-[10px] text-slate-500 leading-tight block">{t.name}</span>
                            </div>
                          </div>
                          <span className="text-[9px] font-mono text-slate-400 shrink-0">{t.durationMs}ms</span>
                        </div>
                      ))}
                    </div>
                  </Card>
                    ...
                    >
                      <div>
                        <div className="flex items-center justify-between gap-2 mb-1.5">
                          <span className="text-[11px] font-medium font-mono px-1.5 py-0.5 rounded bg-slate-100 text-slate-700">
                            #{test.id}
                          </span>
                          <Badge variant="success" size="sm">
                            {test.badge}
          ...
          {lastResponse && (
            <Card variant="default" padding="sm" className="mt-4 text-xs flex flex-wrap items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <span className="font-semibold text-slate-700">Latest Test:</span>
                <Badge
                  variant={lastResponse.status === "SUCCESS" ? "success" : "danger"}
                  size="sm"
                >
                  {lastResponse.status}
                </Badge>
              ...
              {lastResponse.fallback_applied && (
                <Badge variant="warning" size="sm">
                  Fallback: {lastResponse.fallback_reason || "Automatic Safe Failover"}
                </Badge>
              )}
            </Card>
          )}
