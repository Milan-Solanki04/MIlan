...
import { InteractiveJourneyMapView } from './components/InteractiveJourneyMapView';
...
export default function App() {
  ...
  const [previousTab, setPreviousTab] = useState<NavigationTab | null>(null);
  ...
  const handleGoBack = () => {
    if (previousTab && previousTab !== activeTab) {
      const target = previousTab;
      setPreviousTab(activeTab);
      setActiveTab(target);
      window.scrollTo({ top: 0, behavior: 'smooth' });
    } else {
      handleNavigateTab('planner');
    }
  };
  ...
  useEffect(() => {
    if (!originPlace.name && !destPlace.name) return;
  ...
  const runRouteSearch = async () => {
    if (!originPlace?.name?.trim() || !destPlace?.name?.trim()) {
      setRoutes([]);
  ...
  return (
    <div id="smart-route-planner-app" className="min-h-screen bg-slate-50 text-slate-900 font-sans antialiased pb-20">
      ...
      {/* Traveler Journey Breadcrumb & Guidance Bar */}
      <JourneyBreadcrumb
        activeTab={activeTab}
        ...
        onBack={handleGoBack}
        contextInfo={{
          origin: selectedRoute?.origin_name || bookedTrips[0]?.origin_name,
          destination: selectedRoute?.destination_name || bookedTrips[0]?.destination_name,
          tripRef: bookedTrips[0]?.booking_ref,
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        ...
        {activeTab === 'disruptions' && (
          <div id="disruptions-tab-view">
            <DisruptionConciergeView
              ...
              onSwitchToPlanner={() => handleNavigateTab('planner')}
            />
          </div>
        )}
        ...
        {/* TAB 5: Interactive Journey Map */}
        {activeTab === 'map' && (
          <div id="map-tab-view">
            <InteractiveJourneyMapView
              bookedTrips={bookedTrips}
              activeRoute={selectedRoute || routes[0] || null}
              onNavigateToTab={(tab) => handleNavigateTab(tab as any)}
              onOpenInConcierge={(origin, dest) =>
                handleLaunchInConcierge(
                  origin,
                  dest,
                  'Corridor disruption simulated from Map'
                )
              }
            />
          </div>
        ...
        {/* TAB 6: Find Help & 24/7 Helplines */}
        {activeTab === 'help' && (
          <div id="help-tab-view">
            <FindHelpView
              onLaunchConcierge={(orig, dest, disr) =>
                handleLaunchInConcierge(
                  orig || 'Mumbai Central',
                  dest || 'Goa',
                  disr || 'Train cancelled'
                )
              }
              onNavigateToTab={(tab) => handleNavigateTab(tab as any)}
            />
        ...
        {/* TAB 7: Notifications & Real-Time Alerts (Part D) */}
