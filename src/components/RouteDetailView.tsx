...
export const RouteDetailView: React.FC<RouteDetailViewProps> = ({
  ...
  return (
    <Card id="route-detail-container" variant="default" padding="lg" className="mb-6">
      {/* Header bar */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-border">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-slate-500 font-mono">• {route.total_distance_km} km total</span>
          </div>
      ...
      <div id="disruption-simulator-card" className="my-4 p-3.5 sm:p-4 rounded-lg bg-amber-50/60 border border-amber-200/80">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <select
              ...
              className="text-xs bg-white border border-amber-300/80 rounded-md px-2.5 py-1.5 font-medium text-slate-800 focus:outline-none focus:ring-1 focus:ring-amber-500"
            >
            ...
            <div className="flex items-center gap-1 bg-white border border-amber-300/80 rounded-md px-2 py-1">
              ...
              <input
                ...
                className="w-10 text-xs font-semibold text-slate-900 focus:outline-none font-mono"
              />
              <span className="text-[11px] text-slate-500 font-mono">min</span>
            </div>
          ...
          {route.segments.map((seg, idx) => {
            ...
            return (
              <div key={seg.id || idx} className="relative group">
                {/* Node icon */}
                <div className="absolute -left-6 sm:-left-8 top-1 w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-surface-elevated border-2 border-slate-900 flex items-center justify-center shadow-2xs">
                  {getModeIcon(seg.mode)}
                </div>

                {/* Segment Content Card */}
                <div className="p-3.5 sm:p-4 rounded-lg border border-border bg-slate-50/50 hover:bg-slate-50 transition-colors">
                  <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      {seg.flight_or_service_num && (
                        <span className="px-1.5 py-0.5 rounded text-[10px] font-semibold bg-slate-200 text-slate-700 font-mono">
                          {seg.flight_or_service_num}
                        </span>
                      )}
                      {seg.cabin_class && (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 my-2 text-xs">
                    <div>
                      ...
                      <div className="font-semibold text-slate-900 mt-0.5 font-mono">{seg.departure_time}</div>
                      <div className="text-slate-600">{seg.start_name} {seg.start_code ? `(${seg.start_code})` : ''}</div>
                    </div>

                    <div>
                      <div className="text-[10px] uppercase font-semibold text-slate-400">Arrival</div>
                      <div className="font-semibold text-slate-900 mt-0.5 font-mono">{seg.arrival_time}</div>
                  ...
                  {/* Instructions */}
                  {seg.instructions && (
                    <div className="mt-2 text-xs text-slate-500 bg-white p-2.5 rounded-md border border-border">
                      <span className="font-medium text-slate-700">Transit Instructions: </span>
                      {seg.instructions}
                    </div>
                  )}

                  {/* Scheduled Intermediate Stops & Halts */}
                  {seg.intermediate_stops && seg.intermediate_stops.length > 0 && (
                    <div className="mt-3 bg-white rounded-lg border border-border overflow-hidden">
                      <button
                        type="button"
                      ...
                      {expandedStops[seg.id] !== false && (
                        <div className="p-3 border-t border-border divide-y divide-border bg-white">
                            ...
                            <div key={sIdx} className="flex items-center justify-between py-2 text-xs text-slate-700">
                              ...
                              <div className="flex items-center gap-2.5 text-[11px] text-slate-500 font-mono">
                                ...
                                {stop.halt_minutes && stop.halt_minutes > 0 ? (
                                  <span className="bg-amber-50 text-amber-800 border border-amber-200/60 px-1.5 py-0.5 rounded font-medium font-sans">
                                    {stop.halt_minutes}m halt
                                  </span>
                                ) : null}
                                {stop.distance_km ? (
                                  <span className="text-slate-400 font-medium font-sans">{stop.distance_km} km</span>
                                ) : null}
                              </div>
                            </div>
                          ))}
                  ...
                  {hasLayover && (
                    <div className="mt-3 py-1.5 px-3 rounded-md bg-slate-100 flex items-center justify-between text-xs text-slate-700">
      ...
      <div className="mt-6 pt-4 border-t border-border grid grid-cols-1 md:grid-cols-2 gap-3.5">
        {/* Policy evaluation box */}
        <div className="p-3.5 rounded-lg border border-border bg-slate-50/50">
        ...
        <div className="p-3.5 rounded-lg border border-border bg-slate-50/50">
          ...
          <div className="text-xs text-slate-700 space-y-1.5 font-mono">
            <p>
              <strong className="font-sans">Total Itinerary Fare: </strong> {route.currency_symbol || '$'}{(route.total_cost ?? route.total_cost_usd).toLocaleString()} {route.currency || 'USD'}
            </p>
            <p>
              <strong className="font-sans">Policy Benchmark Limit: </strong> {route.currency_symbol || '$'}{(route.benchmark_cost ?? route.benchmark_cost_usd).toLocaleString()} {route.currency || 'USD'}
            </p>
            <p className={`font-sans ${(route.total_cost ?? route.total_cost_usd) <= (route.benchmark_cost ?? route.benchmark_cost_usd) ? 'text-emerald-700 font-medium' : 'text-amber-700 font-medium'}`}>
