  ...
  return (
    <div ref={wrapperRef} className="relative w-full">
      ...
      <div className="relative flex items-center">
        {onOpenMapPicker ? (
          <button
            ...
            className="absolute left-2.5 text-slate-400 hover:text-blue-600 p-1 rounded-md transition-colors z-10 cursor-pointer group hover:bg-slate-100"
          >
            {isLoading ? (
              <Loader2 className="w-4 h-4 animate-spin text-slate-500" />
            ) : (
              <MapPin className="w-4 h-4 text-slate-500 group-hover:text-blue-600 transition-colors" />
            )}
          </button>
        ) : (
        ...
        <input
          ...
          className="w-full pl-9 pr-8 py-2 bg-slate-50 border border-slate-200 rounded-md text-xs font-semibold text-slate-900 placeholder:text-slate-400 focus:bg-white focus:outline-none focus:ring-1 focus:ring-slate-900 transition-colors"
        />
        ...
        {inputText && (
          <button
            type="button"
            onClick={handleClear}
            className="absolute right-2 text-slate-400 hover:text-slate-600 p-1 rounded-md"
            title="Clear"
          >
      ...
      {isOpen && predictions.length > 0 && (
        <div className="absolute left-0 right-0 top-full mt-1 bg-white border border-slate-200 rounded-lg shadow-lg z-50 overflow-hidden max-h-60 overflow-y-auto">
          <div className="px-3 py-1.5 bg-slate-50 border-b border-slate-100 text-[10px] uppercase tracking-wider font-semibold text-slate-500 flex items-center justify-between">
            <span>Places Suggestions</span>
            <span className="text-[9px] lowercase font-mono text-slate-400">Google Places API</span>
          </div>
